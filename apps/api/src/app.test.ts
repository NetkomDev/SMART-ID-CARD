...
describe("Core API envelope", () => {
  ...
  it.each(["/api/v1/device/library/visits", "/api/v1/device/library/visits/sync"])(
    "requires device credentials for Phase 11 endpoint %s", async (path) => {
      ...
      const response = await request(createApp()).post(path).send(path.endsWith("sync") ? { events: [event] } : event);
      expect(response.status).toBe(401);
      expect(response.body.error.code).toBe("DEVICE_AUTH_INVALID");
    }
  );
